...
test.describe("ButtonLink", ()  => {
  test("button link in the navbar should go to the shop", async({ page }) => {
    ...
    expect(page.url()).toBe(billetterieLink);
  });

  test("button link in the home section should go to the shop", async({ page }) => {
    await page.goto("https//localhost:3000/");
  ...
  test("button link in the footer should go to the shop", async({ page }) => {
    await page.goto("https//localhost:3000/");
