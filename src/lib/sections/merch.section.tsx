...
const MerchSection = (): ReactElement => {
  return (
    <section id="merch" className="w-screen bg-blueWaves bg-no-repeat bg-cover bg-center py-28 px-12  md:px-44">
      <div className="w-full flex flex-col md:flex-row items-center md:justify-between mb-16">
        <div className="text-white text-3xl md:text-7xl tracking-normal mb-4 md:mb-0">
          <h2>CuicuiteDays</h2>
          <span>collection 2024</span>
        </div>
        <div className="flex md:block flex-col items-center gap-4">
          <p className="text-white text-sm md:text-xl text-center md:text-left max-w-xs md:max-w-lg mb-2 font-normal font-montserrat opacity-85">Découvrez la nouvelle collection 2024 des CuicuiteDays sur notre boutique en ligne</p>
      ...
      <div className="w-full flex flex-col md:flex-row justify-between gap-8">
        <Link href={merchLink} target="_blank" className="relative text-white text-3xl cursor-pointer ease-in-out duration-500 group/sweat hover:scale-105">
          ...
          <div className="absolute bottom-4 left-6 text-xl md:text-3xl">
            <h3>Sweat</h3>
            <span>CuicuiteDays 2024</span>
          </div>
        </Link>
        <img src="./illustration/white-logo.svg" alt="Logo du festival les CuicuiteDays" className="hidden 2xl:block" />
